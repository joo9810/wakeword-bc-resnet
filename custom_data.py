...
LABELS = ['silence', 'unknown', 'wakeword']
...
UNKNOWN_SAMPLE_PER_FOLDER = 100  # 폴더당 최대 샘플 수
VAL_RATIO = 0.1
SEED = 42
...
def build_file_list(val_ratio=VAL_RATIO, seed=SEED):
    ...
    # 1. wakeword
    wakeword_files = []
    wakeword_files1 = get_wavs(WAKEWORD_ROOT1)
    wakeword_files2 = get_wavs(WAKEWORD_ROOT2)
    wakeword_files3 = get_wavs(WAKEWORD_ROOT3)
    wakeword_files4 = get_wavs(WAKEWORD_ROOT4)
    wakeword_files.extend(wakeword_files1)
    wakeword_files.extend(wakeword_files2)
    wakeword_files.extend(wakeword_files3)
    wakeword_files.extend(wakeword_files4)
    print(f"wakeword: {len(wakeword_files)}개")

    # 2. unknown (AI_Hub 각 폴더에서 최대 100개씩)
    unknown_files = []

    all_words = sorted(os.listdir(AI_HUB_ROOT1))
    for word in all_words:
        word_path = os.path.join(AI_HUB_ROOT1, word)
    ...
    all_words = sorted(os.listdir(AI_HUB_ROOT2))
    for word in all_words:
        word_path = os.path.join(AI_HUB_ROOT2, word)
        if os.path.isdir(word_path):
            wavs = get_wavs(word_path)
            sample_size = min(UNKNOWN_SAMPLE_PER_FOLDER, len(wavs))
            unknown_files.extend(random.sample(wavs, sample_size))
    
    all_words = sorted(os.listdir(AI_HUB_ROOT3))
    for word in all_words:
        word_path = os.path.join(AI_HUB_ROOT3, word)
        if os.path.isdir(word_path):
            wavs = get_wavs(word_path)
            sample_size = min(UNKNOWN_SAMPLE_PER_FOLDER, len(wavs))
            unknown_files.extend(random.sample(wavs, sample_size))

    noise_speech_files = get_wavs(NOISE_SPEECH_ROOT)
    noise_both_files   = get_wavs(NOISE_BOTH_ROOT)
    hard_negative_files = get_wavs(HARD_NEGATIVE_ROOT)
    unknown_files.extend(noise_speech_files)
    unknown_files.extend(noise_both_files)
    unknown_files.extend(hard_negative_files)
    print(f"unknown ({len(all_words)}개 단어 × {UNKNOWN_SAMPLE_PER_FOLDER}개): {len(unknown_files)}개")

    # 3. silence
    silence_files = get_wavs(SILENCE_ROOT)
    vad_silence_files = get_wavs(VAD_SILENCE_ROOT)
    silence_files.extend(vad_silence_files)
    ...
    train_files = (
        ...
        [(f, _label_to_idx['unknown'])  for f in unk_train] +
        [(f, _label_to_idx['silence'])  for f in sil_train]
    )
    val_files = (
        [(f, _label_to_idx['wakeword']) for f in ww_val] +
        [(f, _label_to_idx['unknown'])  for f in unk_val] +
        [(f, _label_to_idx['silence'])  for f in sil_val]
    )
    ...
    print(f"  unknown:  {len(unk_train)}개")
    print(f"  silence:  {len(sil_train)}개")
    print(f"  합계:     {len(train_files)}개")
    print(f"\n[Validation]")
    print(f"  wakeword: {len(ww_val)}개")
    print(f"  unknown:  {len(unk_val)}개")
    print(f"  silence:  {len(sil_val)}개")
