...
def evaluate(model, loader, device):
    ...
    with torch.no_grad():
        for data, target in loader:
            ...
            output = model(data)
            pred   = output.view(-1, N_CLASS).argmax(dim=-1)
...
if __name__ == "__main__":
    ...
    # ================================
    print("\n데이터 로드 중...")
    train_files, val_files = custom_data2.build_file_list()

    train_dataset = custom_data2.CustomAudioDataset(train_files, is_training=True)
    val_dataset   = custom_data2.CustomAudioDataset(val_files,   is_training=False)

    train_loader = DataLoader(
        train_dataset, batch_size=BATCH_SIZE,
        shuffle=True, collate_fn=custom_data2.collate_fn,
        num_workers=2
    )
    val_loader = DataLoader(
        val_dataset, batch_size=BATCH_SIZE,
        shuffle=False, collate_fn=custom_data2.collate_fn,
        num_workers=2
    )
    ...
    for epoch in range(N_EPOCH):
        ...
        for data, target in loop:
            ...
            target = target.to(DEVICE)
            output = model(data)
            loss   = F.nll_loss(output.view(-1, N_CLASS), target)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
