...
MODEL_PATH  = './best_horiya2_1.pt'
SCALE       = 2
N_CLASS     = 3
CLASSES     = ['silence', 'unknown', 'wakeword']
THRESHOLD   = 0.8      # 웨이크워드 감지 임계값
...
def audio_callback(indata, frames, time_info, status):
    ...
    global last_inference_time, detected_count
    ...
    silence_prob  = probs[CLASSES.index('silence')]
    unknown_prob  = probs[CLASSES.index('unknown')]

    # 결과 출력
    bar_w = "█" * int(wakeword_prob * 20)
    bar_u = "█" * int(unknown_prob  * 20)
    bar_s = "█" * int(silence_prob  * 20)
    ...
    if wakeword_prob >= THRESHOLD:
        ...
        print(
            ...
            f"unknown {unknown_prob*100:5.1f}% {bar_u:<20} | "
            f"silence {silence_prob*100:5.1f}% {bar_s:<20}",
            end='', flush=True
        )
